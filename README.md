# housepainter

![Built with Pollinations](https://img.shields.io/badge/Built%20with-Pollinations-8a2be2?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&labelColor=6a0dad)

Upload a photo of your house and see it repainted in any color using AI.

**Live demo:** https://konsumer.github.io/housepainter

## Features

- Upload a photo of your house
- **Single color** mode — pick any color and paint the whole house
- **Palette mode** — choose a seed color and a harmony style (analogic, triad, complement, etc.) to generate a 3-color scheme (primary, secondary, trim) via [The Color API](https://www.thecolorapi.com)
- AI image editing powered by [pollinations.ai](https://pollinations.ai)

## Usage

1. Click **Login** to authenticate with pollinations.ai
2. Upload a photo of your house
3. Choose **Single Color** or **Palette** mode
4. Pick your color(s) and click **PAINT**

## Development

```sh
npm install
npm start
```

Runs on port 51730. Requires a pollinations.ai account for the API key.

## Stack

- [React](https://react.dev)
- [Vite](https://vite.dev)
- [Tailwind CSS](https://tailwindcss.com) + [DaisyUI](https://daisyui.com)
- [pollinations.ai](https://pollinations.ai) — AI image editing
- [The Color API](https://www.thecolorapi.com) — palette generation
