...
async function fetchPalette(hex, mode) {
  ...
  const res = await fetch(`https://www.thecolorapi.com/scheme?hex=${clean}&mode=${mode}&count=3`)
...
function ColorSwatch({ color, label }) {
  return (
    <div className='flex flex-col items-center gap-2'>
      <div className='w-16 h-16 rounded border-2 border-base-content/30 shadow-lg' style={{ background: color.hex }} title={color.name} />
      <div className='badge badge-outline text-xs'>{label}</div>
      <div className='text-xs font-mono font-bold'>{color.hex}</div>
      <div className='text-xs text-center text-base-content/60 max-w-16 leading-tight'>{color.name}</div>
    </div>
  )
}
...
export default function App() {
  ...
  if (!localStorage.polykey) {
    return (
      <div data-theme='cyberpunk' className='min-h-screen bg-base-200 flex items-center justify-center p-6'>
        <div className='card w-full max-w-md bg-base-100 card-neon'>
          <div className='card-body items-center text-center gap-6'>
            ...
            <div>
              <h1 className='card-title text-3xl font-black tracking-tight justify-center text-primary'>HOUSEPAINTER</h1>
              <p className='text-base-content/50 text-sm mt-1 font-mono'>AI-powered exterior color visualizer</p>
            </div>
            <div className='divider divider-primary my-0 opacity-30' />
            <p className='text-base-content/70 leading-relaxed'>Upload a photo of your house and see how it looks in any color — powered by AI image editing.</p>
            <div className='card-actions w-full'>
              <a href={`https://enter.pollinations.ai/authorize?${params}`} className='btn btn-primary w-full'>
  ...
  const canPaint = imageIn && !painting && (colorMode === 'single' || (colorMode === 'palette' && palette))
  ...
  return (
    <div data-theme='cyberpunk' className='min-h-screen bg-base-200'>
      ...
      <div
        ...
        style={{
          boxShadow: '0 1px 20px -4px color-mix(in oklch, var(--color-primary) 40%, transparent)'
        }}
      >
        <div className='flex items-center gap-3 text-primary'>
          <HouseIcon />
          <div>
            <span className='text-xl font-black tracking-tight'>HOUSEPAINTER</span>
            <span className='text-xs text-base-content/40 block leading-none'>AI color visualizer</span>
          </div>
        </div>
      </div>
      ...
      <div className='p-6 max-w-4xl mx-auto flex flex-col gap-6'>
        ...
        <div className='card bg-base-100 card-neon'>
          <div className='card-body gap-4'>
            ...
            <input type='file' className='file-input file-input-primary w-full' onChange={handleImageUpload} accept='image/*' />
          </div>
        </div>
            ...
            <div className='flex flex-col gap-4'>
              ...
              <div className='card bg-base-100 card-neon'>
                <div className='card-body gap-3'>
                  <h2 className='card-title text-base text-primary'>Color Mode</h2>
                  ...
                  {colorMode === 'single' && (
                    <div className='flex items-center gap-4 pt-1'>
                      <label className='text-sm text-base-content/70 font-medium'>Paint color</label>
                      <div className='flex items-center gap-3'>
                        ...
                        />
                        <span className='font-mono text-sm'>{colorIn.toUpperCase()}</span>
                      </div>
                    </div>
                  )}
                  ...
                  {colorMode === 'palette' && (
                    <div className='flex flex-col gap-3 pt-1'>
                      <div className='flex items-center gap-4'>
                        <label className='text-sm text-base-content/70 font-medium'>Seed color</label>
                        <div className='flex items-center gap-3'>
                          <input
                            type='color'
                            value={seedColor}
                            ...
                            className='w-12 h-10 rounded cursor-pointer border border-base-content/20 bg-transparent p-0.5'
                          />
                          <span className='font-mono text-sm'>{seedColor.toUpperCase()}</span>
                        </div>
                      </div>

                      <div className='flex flex-col gap-1'>
                        <label className='text-sm text-base-content/70 font-medium'>Scheme style</label>
                        <select
                      ...
                      <button className='btn btn-accent btn-sm' onClick={handleFetchPalette} disabled={loadingPalette}>
                        {loadingPalette && <span className='loading loading-spinner w-4 h-4' />}
              ...
              <div className='card bg-base-100 card-neon'>
                <div className='card-body gap-2'>
                  <h2 className='card-title text-base text-primary'>Prompt</h2>
                  <textarea value={primaryPrompt} onChange={(e) => primaryPromptSet(e.target.value)} className='textarea textarea-bordered w-full resize-none' placeholder='Describe what you want the AI to do…' rows={4} />
                </div>
              </div>
              ...
              <div className='card bg-base-100 card-neon'>
                <div className='card-body gap-2'>
                  <h2 className='card-title text-base text-primary'>Additional Instructions</h2>
                  <textarea value={additionalPrompt} onChange={(e) => additionalPromptSet(e.target.value)} className='textarea textarea-bordered w-full resize-none' placeholder='e.g. keep the shutters dark, make the door red…' rows={3} />
                </div>
              </div>
              ...
              {/* Paint button */}
              <button onClick={handlePaint} disabled={!canPaint} className='btn btn-primary btn-lg w-full'>
                {painting && <span className='loading loading-spinner w-5 h-5' />}
                {painting ? 'Painting…' : 'Paint My House'}
              </button>
              {colorMode === 'palette' && !palette && <p className='text-xs text-base-content/40 text-center -mt-2'>Generate a palette first</p>}
            </div>
            ...
            <div className='flex flex-col gap-4'>
              <div className='card bg-base-100 card-neon'>
                <div className='card-body gap-3 p-4'>
                  <h2 className='card-title text-base text-primary'>Original</h2>
                  <img src={imageIn} alt='Your house' className='rounded w-full object-cover' />
                </div>
              </div>
              ...
              {imageOut && (
                <div className='card bg-base-100 card-neon-result'>
                  <div className='card-body gap-3 p-4'>
                    <h2 className='card-title text-base text-secondary'>Result</h2>
                    <img src={imageOut} alt='Painted house' className='rounded w-full object-cover' />
                    <a href={imageOut} download='painted-house.jpg' className='btn btn-outline btn-primary btn-sm'>
                      Download
                    </a>
                  </div>
                </div>
              ...
              {painting && (
                <div className='card bg-base-100 card-neon'>
                  <div className='card-body items-center gap-4 py-10'>
                    ...
                    <p className='text-base-content/50 text-sm'>AI is painting your house…</p>
                  </div>
                </div>
              )}
            </div>
